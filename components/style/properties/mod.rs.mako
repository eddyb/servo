    ...
    <%def name="single_keyword_computed(name, values, experimental=False)">
        <%self:single_component_value name="${name}" experimental="${experimental}">
		    ...
		    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                        match self {
                            % for value in values.split():
                                &${to_rust_ident(value)} => write!(f, "${value}"),
                            % endfor
                        }
                    }
                }
...
//            }
    ...
    <%self:single_component_value name="line-height">
        ...
        pub enum SpecifiedValue {
            ...
            SpecifiedPercentage(CSSFloat),
        }
        impl fmt::Show for SpecifiedValue {
            fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                match self {
                    &SpecifiedNormal => write!(f, "normal"),
                    &SpecifiedLength(length) => write!(f, "{}", length),
                    &SpecifiedNumber(number) => write!(f, "{}", number),
		    &SpecifiedPercentage(number) => write!(f, "{}%", number * 100.),
                }
            }
        }
        ...
        pub fn from_component_value(input: &ComponentValue, _base_url: &Url)
            ...
            match input {
                ...
                => Ok(SpecifiedPercentage(value.value / 100.)),
        ...
        pub fn to_computed_value(value: SpecifiedValue, context: &computed::Context)
            ...
            match value {
                ...
                SpecifiedPercentage(value) => Length(computed::compute_Au(specified::Em(value), context)),
            }
        }
    </%self:single_component_value>
    ...
    <%self:single_component_value name="vertical-align">
        ...
        impl fmt::Show for SpecifiedValue {
            fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                match self {
                    % for keyword in vertical_align_keywords:
                        &Specified_${to_rust_ident(keyword)} => write!(f, "${keyword}"),
                    % endfor
		    &SpecifiedLengthOrPercentage(lop) => write!(f, "{}", lop),
                }
            }
        }
    ...
    <%self:raw_longhand name="color">
        ...
        pub fn parse_specified(input: &[ComponentValue], _base_url: &Url)
            ...
            match one_component_value(input).and_then(CSSColor::parse) {
                Ok(CSSColor { parsed: RGBAColor(rgba), authored }) => {
                    let rgba = CSSRGBA {
                        parsed: rgba,
                        authored: authored,
                    };
                    Ok(SpecifiedValue(rgba))
                }
                Ok(CSSColor { parsed: CurrentColor, .. }) => Ok(Inherit),
                Err(()) => Err(()),
            }
        }
    </%self:raw_longhand>
...
//                Monospace,
            ...
            impl fmt::Show for FontFamily {
                fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                    match self {
                        &FamilyName(ref name) => write!(f, "{}", name),
                    }
                }
            }
...
//                    }
    ...
    <%self:single_component_value name="font-weight">
        ...
        impl fmt::Show for SpecifiedValue {
            fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                match self {
                    &Bolder => write!(f, "bolder"),
                    &Lighter => write!(f, "lighter"),
                    % for weight in range(100, 901, 100):
                        &SpecifiedWeight${weight} => write!(f, "{}", ${weight}i),
                    % endfor
                }
            }
        }
...
impl PropertyDeclaration {
    pub fn name(&self) -> String {
        match self {
            % for property in LONGHANDS:
                % if property.derived_from is None:
                    &${property.camel_case}Declaration(..) => "${property.name}".to_string(),
		% endif
            % endfor
            _ => "".to_string(),
        }
    ...
    pub fn matches(&self, name: &str) -> bool {
        ...
        match (self, name_lower.as_slice()) {
            % for property in LONGHANDS:
                % if property.derived_from is None:
                    (&${property.camel_case}Declaration(..), "${property.name}") => true,
		% endif
            % endfor
            _ => false,
        }
            ...
            match *declaration {
                ...
                ColorDeclaration(ref value) => {
                    context.color = match *value {
                        SpecifiedValue(ref specified_value) => specified_value.parsed,
                        Initial => longhands::color::get_initial_value(),
                        Inherit => inherited_style.get_color().color.clone(),
